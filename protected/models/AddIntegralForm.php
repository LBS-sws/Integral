 ...
 */
class AddIntegralForm extends CFormModel
{
	/* User Fields */
	public $id = 0;
	public $employee_id;
	...
	public function attributeLabels()
	{
		return array(
            ...
            'activity_id'=>Yii::t('integral','Credit activities Name'),
	...
	{
		return array(
			array('id, employee_id, activity_id, employee_name, set_id, integral, images_url, remark, reject_note, lcu, luu, lcd, lud','safe'),
	...
	public function validateActivity($attribute, $params){
        $rows = Yii::app()->db->createCommand()->select("*")->from("gr_act_add")
            ->where("id=:id", array(':id'=>$this->activity_id))->queryRow();
        if ($rows){
            $date = date("Y-m-d");
            if (strtotime($date)<strtotime($rows["start_time"])||strtotime($date)>strtotime($rows["end_time"])){
                $message = "活動已經過期";
                $this->addError($attribute,$message);
            }
        }else{
            $message = Yii::t('integral','Credit activities Name'). Yii::t('integral',' Did not find');
            $this->addError($attribute,$message);
        }
    }

	public function validateIntegral($attribute, $params){
        $rows = Yii::app()->db->createCommand()->select("*")->from("gr_integral_add")
            ->where("id=:id", array(':id'=>$this->set_id))->queryRow();
        if ($rows){
            if($rows["year_sw"]==1){
                $count = Yii::app()->db->createCommand()->select("*")->from("gr_gral_add")
                    ->where("set_id=:set_id and employee_id=:employee_id and activity_id=:activity_id and state in (1,3)",
                        array(':set_id'=>$this->set_id,':employee_id'=>$this->employee_id,':activity_id'=>$this->activity_id))->queryScalar();
                if($count > $rows["year_num"]){
                    $message = "该学分每年申請次數不能大于".$rows["year_num"];
                    $this->addError($attribute,$message);
                }
            }else{
	...
	public function validateDelete(){
        $rows = Yii::app()->db->createCommand()->select()->from("gr_gral_add")
	...
	public function retrieveData($index)
	{
        ...
        $suffix = Yii::app()->params['envSuffix'];
        $city_allow = Yii::app()->user->city_allow();
        $rows = Yii::app()->db->createCommand()->select("a.*,d.integral_type,d.s_remark,docman$suffix.countdoc('GRAL',a.id) as graldoc")->from("gr_gral_add a")
            ->leftJoin("hr$suffix.hr_employee b","a.employee_id = b.id")
	...
	protected function saveStaff(&$connection)
	{
		...
		switch ($this->scenario) {
			case 'delete':
                $sql = "delete from gr_gral_add where id = :id and city IN ($city_allow)";
				break;
			case 'new':
				$sql = "insert into gr_gral_add(
							employee_id, activity_id, set_id, integral, remark, state, city, lcu
						) values (
							:employee_id, :activity_id, :set_id, :integral, :remark, :state, :city, :lcu
						)";
				break;
			case 'edit':
				$sql = "update gr_gral_add set
		...
		if (strpos($sql,':activity_id')!==false)
			$command->bindParam(':activity_id',$this->activity_id,PDO::PARAM_INT);
