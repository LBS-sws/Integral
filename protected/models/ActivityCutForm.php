...
class ActivityCutForm extends CFormModel
{
	public $id;
	public $name;
	public $start_time;
	...
	{
		return array(
            'name'=>Yii::t('integral','Cut activities Name'),
	...
	public function validateName($attribute, $params){
        ...
        $rows = Yii::app()->db->createCommand()->select("id")->from("gr_act_cut")
	...
	public function retrieveData($index) {
		$rows = Yii::app()->db->createCommand()->select("*")
            ->from("gr_act_cut")->where("id=:id",array(":id"=>$index))->queryAll();
		if (count($rows) > 0) {
			foreach ($rows as $row) {
                $this->id = $row['id'];
                $this->name = $row['name'];
    ...
    //獲取積分類型列表
    public function getActivityCutNameToId($id){
        $rs = Yii::app()->db->createCommand()->select("name")->from("gr_act_cut")->where("id=:id",array(":id"=>$id))->queryRow();
        if($rs){
            return $rs["name"];
        }
        return $id;
    ...
    public function deleteValidate(){
        $rs0 = Yii::app()->db->createCommand()->select()->from("gr_gral_cut")->where("activity_id=:activity_id",array(":activity_id"=>$this->id))->queryAll();
        if($rs0){
            return false;
        }else{
            return true;
	...
	protected function saveGoods(&$connection) {
        ...
        switch ($this->scenario) {
            case 'delete':
                $sql = "delete from gr_act_cut where id = :id";
                break;
            case 'new':
                $sql = "insert into gr_act_cut(
